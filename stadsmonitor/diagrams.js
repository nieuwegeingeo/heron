 ...
 *
 * Ext.get("hr-diagram").dom.innerHTML
 *
 **/

Heron.widgets.Diagram =
		...
		(function () { // Creates and runs anonymous function, its result is assigned to Singleton
			...
			var instance = {
				...
				removeBookmark: function (contextid, id) {
					// get element id
					if (confirm('Weet u zeker dat u deze kaart wilt verwijderen?')){
						var elmm = Ext.getCmp(contextid);
 ...
 *  the point (x,y) where the map should be centered.
 */
Heron.widgets.DiagramPanel = Ext.extend(Heron.widgets.HTMLPanel, {
	...
	initComponent: function () {
		...
		Heron.widgets.DiagramPanel.superclass.initComponent.call(this);
	...
	getHtml: function () {
		...
		var htmllines = '<div class="hr-diagrams-panel-body">';
		...
		*/
		...
		htmllines += '<div id="hr-diagram"></div>';
...
/** api: xtype = hr_diagrams */
Ext.reg('hr_diagrams', Heron.widgets.DiagramPanel);

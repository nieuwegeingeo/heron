...
Ext.extend(Heron.data.OpenLS_XLSReader, Ext.data.XmlReader, {
    ...
    */
    ...
    // RD addition to also be anble to retrn attribute values of an element
    // Note: if node contains 3 child nodes text of all three will be returned
    addOptXlsText: function(format, text, node, tagname, sep, attr) {
		var elms = format.getElementsByTagNameNS(node, "http://www.opengis.net/xls", tagname);
        var txt = '';
		if (elms) {
			Ext.each(elms, function(elm, index) {
                // only if attr is given as paramter
                if (typeof attr != "undefined")
                {
                    var str = format.getAttributeNS(elm, '', attr);
                    if (str) {
                        //text = text + sep + str;
                        txt = txt + sep + str;
                    }
	...
	extractData: function(root) {
		...
		Ext.each(addresses, function(address, index) {
            ...
            var place = reader.addOptXlsText(format, text, address, 'Place', ', ');
            if (place.length>0){
                text = place + text;
                zoom = 11
            }

            text = reader.addOptXlsText(format, text, address, 'Building', '', 'subdivision') + text;
            var number = reader.addOptXlsText(format, text, address, 'Building', ' ', 'number');
            if (number.length>0){
                text = number + text;
                zoom = 16;
            }

            var street = reader.addOptXlsText(format, text, address, 'Street', '');
            if (street.length>0){
                text = street + text;
                // only if zoom isn't already changed by finding a number
                if(zoom<14){zoom=14};
            }

...
// ----------
    ...
    render: function(bulkRender) {
        ...
        if (a.nodeType == "gx_layer") {
            ...
            var metadata = Ext.DomHelper.insertBefore(cb,
                ['<button type="button" id="ext-gen154" class="',cls,'">&nbsp;</button>'].join(""));           
        }
    ...
    onClick: function(e) {      
        //clicking ont he checkbox
        if(e.getTarget('.x-tree-node-cb', 1)) {
            this.toggleCheck(this.isChecked());
        } 
        // clicking on the layername
        else if(e.getTarget('.x-tree-node-anchor', 2))
        {
            //var cb = Ext.get(e.getTarget('.x-tree-node-anchor', 2).previousSibling);
            //cb.toggleCheck(cb.isChecked());
            // not working...
            //console.log('Clicking layername... ignoring');
        }
        else {
            if (this.node.layer.metadata.url || this.node.layer.metadata.html) {
                var metadata = ""
                ...
                if (layer.metadata && layer.metadata.url){
                    metadata += '<br/><p><a href="'+layer.metadata.url+'" target="_blank" >Open metadata url in Browser</a></p>';
                }
                if (metadata == ''){

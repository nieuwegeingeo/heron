...
// ----------
 ...
 /**
     * 
     * Richard: change the icons of the layer nodes
     */
    render: function(bulkRender) {
        ...
        // add i-button, and gray out if there is no metadata
        if (a.nodeType == "gx_layer") {
            // add secure icon class if this is a 'secure' layer
            this.iconNode.classList.remove('hr-tree-node-icon-layer-raster'); // remove (default) raster icon
            this.iconNode.classList.remove('x-tree-node-icon'); // remove layers icon
            this.iconNode.classList.add('x-tree-ec-icon');
            //this.indentNode.childNodes.splice(this.indentNode.childNodes.length-1,1)
            if (this.node.layer.layerIcons == 'secure'){
                this.iconNode.classList.add('hr-tree-node-icon-layer-secure');
                this.iconNode.setAttribute('title', 'Beveiligde laag');
            }
            var cb = this.checkbox;
            
            // Metadata part
            var cls = 'x-btn-text x-tree-elbow-end icon-getfeatureinfo'
            // okt 2019 RD: adding this class makes the little 'i' 30% opaque, we can not do this anymore
            // because IF a layer has metadata is now configured externally (flask metadata app), not 
            // in the config anymore (which was used below)
            /*if (!(this.node.layer.metadata.url || this.node.layer.metadata.html)) {
                cls += ' third_opaque';
            }*/
            var metadataButton = Ext.DomHelper.insertBefore(cb,
                ['<button type="button" class="', cls, '" title="Laaginformatie">&nbsp;</button>'].join(""));
            var theLayer = this.node.layer;
            ...
            // Secure layer part
            if (this.node.layer.layerIcons == 'secure'){
                var secCls = 'x-sec-test-div'
                var secureLayerDiv = Ext.DomHelper.insertBefore(cb.parentNode,
                    ['<div class="',secCls,'"></div>'].join(""));

                function checkGeoserver(e) {
                    var i;
                    //console.log(e.target);
                    var clickNodes = e.target.nextSibling.childNodes;
                    var inputNode;
                    for (i = 0; i< clickNodes.length; i++) {
                        if (clickNodes[i].tagName === 'INPUT') {
                            inputNode = clickNodes[i];
                        }
                    }
                    //console.log(inputNode)
                    var layerName = 'secure_layer_check_wa';
                    var req = '//' + location.host +'/geoserverAD/wfs?request=getFeature&typeName=nieuwegein_wa:secure_layer_check_wa&BBOX=134275,448949,134276,448950&WIDTH=10&HEIGHT=10&format=image/png'
                    $(function(){
                        $.ajax({
                            url: req, // '//' + location.host + '/geoserver/wfs?request=getFeature&LAYERS=nieuwegein:' + layerName + '&BBOX=134275,448949,134276,448950&WIDTH=10&HEIGHT=10&format=image/png',
                            statusCode: {
                                401: function() {
                                ...
                                var secList = document.querySelectorAll('.x-sec-test-div');
                                // var secArray = Array.from(secList);
                                // console.log(secArray);
                                // secArray.forEach(function(item) {item.remove()});
                                
                                for (i = 0;i < secList.length; i++) {
                                   secParent = secList[i].parentNode;
                                   secParent.removeChild(secList[i]);
                                }
                                inputNode.click();
                        });
                    });
                }
                secureLayerDiv.addEventListener('click', checkGeoserver)
        ...
        //$.each($('.icon-getfeatureinfo'), function(index, item){ $(item).siblings('img:first .x-tree-ec-icon img.x-tree-elbow').remove()})
    },
});

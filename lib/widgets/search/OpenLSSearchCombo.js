...
Heron.widgets.search.OpenLSSearchCombo = Ext.extend(Ext.form.ComboBox, {
			...
			initComponent: function() {
				...
				this.store = new Ext.data.Store({
                            ...
                            // Richard http://stackoverflow.com/questions/17763154/how-to-send-extra-parameters-when-loading-a-store-to-a-combobox-in-extjs-4
                            autoLoad: false,
						...
						this.store.on('beforeload', function(store, operation) {
                            ...
                            // Richard: add ', nieuwegein' to every zoekterm
                            store.baseParams.zoekterm = store.baseParams.zoekterm+', nieuwegein';
                        });
				...
				if (this.zoom > 0) {
					this.on("select", function(combo, record, index) {
						...
						// zoom in on the location, BUT zoom CAN be part of record data
                        var zoom = this.zoom;
                        if (typeof record.data.zoom != "undefined"){

...
Heron.widgets.search.LocatieserverSearchCombo = Ext.extend(Ext.form.ComboBox, {
			 ...
			 */
			// url: 'https://geodata.nationaalgeoregister.nl/locatieserver/suggest?fq=+type:*&wt=xml',
			url: 'http://geodata.nationaalgeoregister.nl/locatieserver/suggest?fq=+type:*&wt=xml',
			
			/** config property[lookupUrl]
			 *  URL of the Locatieserver lookup service.
			 */
			// lookupUrl: 'https://geodata.nationaalgeoregister.nl/locatieserver/lookup?fq=+type:*&wt=xml',
			lookupUrl: 'http://geodata.nationaalgeoregister.nl/locatieserver/lookup?fq=+type:*&wt=xml',
			...
			initComponent: function() {
				...
				this.store = new Ext.data.Store({
                            ...
                            listeners:{
                                beforeload: function(store, operation, options){
                                    console.log(store.baseParams.q);
                                    store.baseParams.q = store.baseParams.q + " and woonplaatsnaam:Nieuwegein"
                                },
                                 scope: this,
                                 'change': function(a,b){console.log(a);console.log(b)},
                                 'changed': function(a,b){console.log(a);console.log(b)},
                            },
				...
				if (this.zoom > 0) {
					this.on("select", function(combo, record, index) {
						...
						zoom = this.zoom;
                        if (record.data.type){
                            if (record.data.type=='weg'){
                                zoom = 14; // 1:750
                            }
                            else if (record.data.type=='adres'){
                                zoom = 14; // 1:750
                            }
                            else if (record.data.type=='perceel'){
                                zoom = 14; // 1:750
                            }
                            else if (record.data.type=='hectometerpaal'){
                                zoom = 14; // 1:750
                            }
						}
						Ext.Ajax.request({
